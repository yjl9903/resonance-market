  ...
  watch(
    ...
    ([product, target], prev) => {
      ...
      if (source === target) {
        ...
        if (tr) form.resetField('price', { value: tr.basePrice });
      }
    },
    { immediate: true }
  );
  ...
  watch(
    ...
    ([product, target, price], prev) => {
      ...
      if (target === product.city) {
        ...
        if (percent > 0 && percent < 200) form.resetField('percent', { value: [percent] });
      } else {
  ...
  watch(
    ...
    ([product, target, percent], prev) => {
      ...
      if (target === product.city) {
        ...
        if (price > 0) form.resetField('price', { value: price });
      } else {
        if (transaction && transaction.basePrice > 0) {
